...
//                this.recorder.start();
                    ...
                    this.autoReadData(this.tempFile);
    ...
    private void autoReadData(final String fileName) {
        new Thread() {
            @Override
            public void run() {
                try {
                    ...
                    while (state == STATE.MEDIA_RUNNING && recorder.getRecordingState() == AudioRecord.RECORDSTATE_RECORDING) {
                        int count = recorder.read(data, 0, bufferSize);
                        // 如果读取音频数据没有出现错误，就将数据写入到文件
                        if (AudioRecord.ERROR_INVALID_OPERATION != count && count > 0) {
                            try {
